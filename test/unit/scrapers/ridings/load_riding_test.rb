...
class Scrapers::Ridings::LoadRidingsAttributesTest < ActiveSupport::TestCase
  ...
  def test_id
    assert_equal 99, Scrapers::Ridings::LoadRiding.new( { "electoral_district" => 99 } ).run.id
  end
  ...
  def test_province
    alberta = Factory(:province, :name_en => "Alberta")
    assert_equal alberta, Scrapers::Ridings::LoadRiding.new( { "province" => "Alberta" } ).run.province
  end
...
class Scrapers::Ridings::LoadRidingInteractionTest < ActiveSupport::TestCase
  def test_new_riding_record_is_created
    ...
    Scrapers::Ridings::LoadRiding.new( { "electoral_district" => 99 } ).run
    assert_equal 1, Riding.count
  end

  def test_existing_riding_is_not_recreated
    Factory(:riding, :id => 99)
  ...
  def test_attributes_are_updated
    Factory(:riding, :id => 99, :name_en => "William Henry Rutherford Smithwick III")
    riding = Scrapers::Ridings::LoadRiding.new( { "electoral_district" => 99, "name_en" => "Will Smithwick" } ).run
  ...
  def test_error_logging
    ...
    Scrapers::Ridings::LoadRiding.new( { "electoral_district" => 99 }, :logger => mock_logger ).run
  end
end
